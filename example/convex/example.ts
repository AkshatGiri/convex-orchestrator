...
export const listComments = query({
  ...
  handler: async (ctx, args) => {
    return await ctx.runQuery(components.convexOrchestrator.lib.list, {
      targetId: args.targetId,
    });
  },
});
...
export const translateComment = action({
  ...
  handler: async (ctx, args) => {
    return await ctx.runAction(components.convexOrchestrator.lib.translate, {
      baseUrl: BASE_URL,
...
// the component's API:
export const { list, add, translate } = exposeApi(components.convexOrchestrator, {
